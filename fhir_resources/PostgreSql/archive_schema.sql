...
COMMENT ON TABLE insaights_codeable_concept IS 'Lookup table for centralised/decentralised codeable concepts';
COMMENT ON COLUMN insaights_codeable_concept.concept_type IS 'Defines the category of the concept';
COMMENT ON COLUMN insaights_codeable_concept.code IS 'Code value within the concept type';
COMMENT ON COLUMN insaights_codeable_concept.system IS 'URI that identifies the terminology system t';
COMMENT ON COLUMN insaights_codeable_concept.display IS 'Human-readable display name for the code';
COMMENT ON COLUMN insaights_codeable_concept.text IS 'Optional free-text description provided by the user';
COMMENT ON CONSTRAINT insaights_codeable_concept_pkey ON insaights_codeable_concept IS 'Composite primary key ensuring uniqueness within each concept type';
...
COMMENT ON TABLE insaights_organisation IS 'Master table storing healthcare organisations such as hospitals, clinics, and related facilities';
COMMENT ON COLUMN insaights_organisation.org_id IS 'System-generated UUID uniquely identifying the organisation';
COMMENT ON COLUMN insaights_organisation.name IS 'Official registered name of the healthcare organisation';
COMMENT ON COLUMN insaights_organisation.type IS 'Classification of the organisation (e.g., hospital, clinic, diagnostic center)';
COMMENT ON COLUMN insaights_organisation.address IS 'Primary registered address of the organisation';
COMMENT ON COLUMN insaights_organisation.phone IS 'Primary contact phone number for the organisation';
COMMENT ON COLUMN insaights_organisation.email IS 'Official contact email address of the organisation';
COMMENT ON COLUMN insaights_organisation.active IS 'Indicates whether the organisation is currently active (TRUE) or inactive (FALSE)';
...
COMMENT ON TABLE insaights_attachment IS 'FHIR Attachment resource used to store patient-related documents such as ABHA card scans, photos, and supporting files';
COMMENT ON COLUMN insaights_attachment.attachment_id IS 'Unique identifier for the attachment record';
COMMENT ON COLUMN insaights_attachment.content_type IS 'MIME type of the attachment content (e.g., image/jpeg, application/pdf)';
COMMENT ON COLUMN insaights_attachment.content_language IS 'Language of the attachment content as defined in FHIR Attachment.language';
COMMENT ON COLUMN insaights_attachment.url IS 'External URL where the attachment is stored when not persisted in the database';
COMMENT ON COLUMN insaights_attachment.content_data IS 'Binary content of the attachment stored inline (e.g., ABHA card image, patient photo)';
COMMENT ON COLUMN insaights_attachment.title IS 'Human-readable title or label describing the attachment';
COMMENT ON COLUMN insaights_attachment.size IS 'Size of the attachment file in bytes';
COMMENT ON COLUMN insaights_attachment.content_hash IS 'Cryptographic hash of the attachment content used for integrity verification';
COMMENT ON COLUMN insaights_attachment.creation_date IS 'Timestamp indicating when the attachment was created';

------------------------------------
-- 4. insaights_practitioner
------------------------------------
...
COMMENT ON TABLE insaights_practitioner IS 'FHIR Practitioner resource representing healthcare professionals who provide direct patient care';
COMMENT ON COLUMN insaights_practitioner.practitioner_id IS 'Unique system-generated identifier for the practitioner (FHIR Practitioner.id)';
COMMENT ON COLUMN insaights_practitioner.given_name IS 'Practitioner given or first name (FHIR HumanName.given)';
COMMENT ON COLUMN insaights_practitioner.family_name IS 'Practitioner family or last name (FHIR HumanName.family)';
COMMENT ON COLUMN insaights_practitioner.gender IS 'Administrative gender of the practitioner (FHIR Practitioner.gender)';
COMMENT ON COLUMN insaights_practitioner.birth_date IS 'Date of birth of the practitioner';
COMMENT ON COLUMN insaights_practitioner.phone IS 'Primary contact phone number for the practitioner';
COMMENT ON COLUMN insaights_practitioner.email IS 'Professional or official email address of the practitioner';
COMMENT ON COLUMN insaights_practitioner.active IS 'Indicates whether the practitioner is currently active and eligible to provide services';

------------------------------
-- 5. insaights_practitioner_role
------------------------------
CREATE TABLE insaights_practitioner_role (
    practitioner_role_id UUID PRIMARY KEY,
    practitioner_id UUID REFERENCES insaights_practitioner(practitioner_id),
    organization_id UUID REFERENCES insaights_organisation(org_id),
    role TEXT,
    specialty TEXT,
    contact_number TEXT,
    email TEXT,
...
COMMENT ON TABLE insaights_practitioner_role IS 'FHIR PractitionerRole resource defining roles, responsibilities, and organizational associations of healthcare practitioners';
COMMENT ON COLUMN insaights_practitioner_role.practitioner_role_id IS 'Unique identifier for the practitioner role record (FHIR PractitionerRole.id)';
COMMENT ON COLUMN insaights_practitioner_role.practitioner_id IS 'Reference to the practitioner performing this role (FK to insaights_practitioner)';
COMMENT ON COLUMN insaights_practitioner_role.organization_id IS 'Reference to the organization where the practitioner performs this role (FK to insaights_organisation)';
COMMENT ON COLUMN insaights_practitioner_role.role IS 'Role or function of the practitioner within the organization (e.g., Doctor, Nurse, Surgeon)';
COMMENT ON COLUMN insaights_practitioner_role.specialty IS 'Clinical or professional specialty associated with the practitioner role (e.g., Cardiology, Pediatrics)';
COMMENT ON COLUMN insaights_practitioner_role.contact_number IS 'Primary contact phone number associated with this practitioner role';
COMMENT ON COLUMN insaights_practitioner_role.email IS 'Professional contact email associated with this practitioner role';
COMMENT ON COLUMN insaights_practitioner_role.available_start IS 'Start timestamp of the practitioner’s availability period for this role';
COMMENT ON COLUMN insaights_practitioner_role.available_end IS 'End timestamp of the practitioner’s availability period for this role';
COMMENT ON COLUMN insaights_practitioner_role.active IS 'Indicates whether this practitioner role is currently active';
COMMENT ON COLUMN insaights_practitioner_role.notes IS 'Additional notes or descriptive information about the practitioner role';
...
CREATE TABLE insaights_patient (
    ...
    photo_id     UUID REFERENCES insaights_attachment(attachment_id), 
    religion     TEXT,
    annual_income INT,
    nationality  TEXT,
    ...
    managing_organization_id UUID REFERENCES insaights_organisation(org_id),
...
COMMENT ON TABLE insaights_patient IS 'FHIR Patient resource representation storing demographic and administrative patient data';
COMMENT ON COLUMN insaights_patient.patient_id IS 'Unique identifier for the patient resource (FHIR Patient.id); typically a UUID or system-generated identifier';
COMMENT ON COLUMN insaights_patient.name_prefix IS 'Name prefix component (FHIR HumanName.prefix), e.g., Mr, Ms, Dr';
COMMENT ON COLUMN insaights_patient.first_name IS 'Given name / first name (FHIR HumanName.given[0])';
COMMENT ON COLUMN insaights_patient.middle_name IS 'Additional given name / middle name (FHIR HumanName.given[1]), when available';
COMMENT ON COLUMN insaights_patient.last_name IS 'Family name / surname (FHIR HumanName.family)';
COMMENT ON COLUMN insaights_patient.name_suffix IS 'Name suffix component (FHIR HumanName.suffix), e.g., Jr, Sr, PhD';
COMMENT ON COLUMN insaights_patient.full_name IS 'Full display name for presentation (FHIR HumanName.text)';
COMMENT ON COLUMN insaights_patient.active IS 'Indicates whether the patient record is active (FHIR Patient.active)';
COMMENT ON COLUMN insaights_patient.gender IS 'Administrative gender (FHIR Patient.gender), e.g., male, female, other, unknown';
COMMENT ON COLUMN insaights_patient.birth_date IS 'Date of birth (FHIR Patient.birthDate)';
COMMENT ON COLUMN insaights_patient.deceased_boolean IS 'Indicates whether the patient is deceased (FHIR Patient.deceasedBoolean)';
COMMENT ON COLUMN insaights_patient.deceased_dateTime IS 'Date/time of death when known (FHIR Patient.deceasedDateTime)';
COMMENT ON COLUMN insaights_patient.marital_status_code IS 'Marital status code (FHIR Patient.maritalStatus.coding.code)';
COMMENT ON COLUMN insaights_patient.multiple_birth_boolean IS 'Indicates whether the patient is part of a multiple birth (FHIR Patient.multipleBirthBoolean)';
COMMENT ON COLUMN insaights_patient.multiple_birth_integer IS 'Birth order in a multiple birth (FHIR Patient.multipleBirthInteger)';
COMMENT ON COLUMN insaights_patient.photo_id IS 'Reference to an attachment record for the patient photo or document (FK to insaights_attachment)';
COMMENT ON COLUMN insaights_patient.religion IS 'Patient religion captured for administrative or facility-specific workflows (not a core FHIR element)';
COMMENT ON COLUMN insaights_patient.annual_income IS 'Annual income for administrative purposes when collected (non-clinical attribute)';
COMMENT ON COLUMN insaights_patient.nationality IS 'Patient nationality for administrative and reporting purposes';
COMMENT ON COLUMN insaights_patient.created_at IS 'Timestamp when the record was created';
COMMENT ON COLUMN insaights_patient.created_by IS 'Identifier of the user or system that created the record';
COMMENT ON COLUMN insaights_patient.modified_at IS 'Timestamp when the record was last updated';
COMMENT ON COLUMN insaights_patient.modified_by IS 'Identifier of the user or system that last updated the record';
COMMENT ON COLUMN insaights_patient.managing_organization_id IS 'Organisation responsible for maintaining the patient record (FHIR Patient.managingOrganization; FK to insaights_organisation)';

--------------------------------------
-- 7. insaights_patient_identifier
--------------------------------------
CREATE TABLE insaights_patient_identifier (
    ...
    photo_attachment_id UUID REFERENCES insaights_attachment(attachment_id),
    assigner_id UUID REFERENCES insaights_organisation(org_id),
    patient_id UUID NOT NULL REFERENCES insaights_patient(patient_id),
    CONSTRAINT uq_patient_type UNIQUE (patient_id, type),
    CONSTRAINT uq_system_value UNIQUE (system, value)
);

COMMENT ON TABLE insaights_patient_identifier IS 'FHIR Identifier resource storing patient identifiers such as MRN, passport numbers, and other official identifiers';
COMMENT ON COLUMN insaights_patient_identifier.identifier_id IS 'Unique identifier for the identifier record (FHIR Identifier.id)';
COMMENT ON COLUMN insaights_patient_identifier.use IS 'Intended use of the identifier (FHIR Identifier.use), such as official, usual, secondary, or temporary';
COMMENT ON COLUMN insaights_patient_identifier.type IS 'Code describing the type of identifier (FHIR Identifier.type), e.g., medical record number, passport, driver license';
COMMENT ON COLUMN insaights_patient_identifier.system IS 'Namespace or issuing system for the identifier (FHIR Identifier.system), expressed as a URI, OID, or URL';
COMMENT ON COLUMN insaights_patient_identifier.value IS 'Actual identifier value assigned to the patient (FHIR Identifier.value)';
COMMENT ON COLUMN insaights_patient_identifier.period_start IS 'Start date and time from which the identifier is considered valid (FHIR Identifier.period.start)';
COMMENT ON COLUMN insaights_patient_identifier.period_end IS 'End date and time after which the identifier is no longer valid (FHIR Identifier.period.end)';
COMMENT ON COLUMN insaights_patient_identifier.photo_attachment_id IS 'Optional reference to an attachment associated with the identifier, such as an ID card image';
COMMENT ON COLUMN insaights_patient_identifier.assigner_id IS 'Reference to the organisation that issued or assigned the identifier (FHIR Identifier.assigner)';
COMMENT ON COLUMN insaights_patient_identifier.patient_id IS 'Reference to the patient to whom this identifier belongs';
COMMENT ON CONSTRAINT uq_patient_type ON insaights_patient_identifier IS 'Ensures that each patient can have at most one identifier per identifier type.';
COMMENT ON CONSTRAINT uq_system_value ON insaights_patient_identifier IS 'Ensures global uniqueness of an identifier within its issuing system.';

--------------------------------
-- 8. insaights_patient_telecom
--------------------------------
CREATE TABLE insaights_patient_telecom  (
    ...
    period_end TIMESTAMPTZ,
    patient_id UUID NOT NULL REFERENCES insaights_patient(patient_id),
    CONSTRAINT chk_telecom_rank_non_negative CHECK (rank IS NULL OR rank >= 0)
);

COMMENT ON TABLE insaights_patient_telecom IS 'FHIR ContactPoint resource storing telecom details such as phone numbers, email addresses, and other contact information for a patient';
COMMENT ON COLUMN insaights_patient_telecom.telecom_id IS 'Unique identifier for the telecom contact record';
COMMENT ON COLUMN insaights_patient_telecom.system IS 'Type of contact system (FHIR ContactPoint.system), such as phone, email, fax, or URL';
COMMENT ON COLUMN insaights_patient_telecom.value IS 'Actual contact detail value, such as a phone number or email address (FHIR ContactPoint.value)';
COMMENT ON COLUMN insaights_patient_telecom.use IS 'Intended usage of the contact detail (FHIR ContactPoint.use), such as home, work, mobile, temporary, or old';
COMMENT ON COLUMN insaights_patient_telecom.rank IS 'Priority order of this contact detail, where a lower number indicates higher priority (FHIR ContactPoint.rank)';
COMMENT ON COLUMN insaights_patient_telecom.period_start IS 'Start date and time from which this contact detail is considered valid (FHIR ContactPoint.period.start)';
COMMENT ON COLUMN insaights_patient_telecom.period_end IS 'End date and time until which this contact detail is considered valid; NULL indicates the contact is still active (FHIR ContactPoint.period.end)';
COMMENT ON COLUMN insaights_patient_telecom.patient_id IS 'Reference to the patient who owns this telecom contact detail';

--------------------------------
-- 9. insaights_patient_address
--------------------------------
CREATE TABLE insaights_patient_address (
    address_id UUID PRIMARY KEY,
    use TEXT,
    type TEXT,-- postal | physical | both
    text TEXT,
    line1 TEXT, line2 TEXT, line3 TEXT, line4 TEXT,
    city TEXT, district TEXT, state TEXT, postal_code TEXT, country TEXT,
    period_start TIMESTAMPTZ,
    period_end TIMESTAMPTZ,
    patient_id UUID NOT NULL REFERENCES insaights_patient(patient_id)
);

COMMENT ON TABLE insaights_patient_address IS 'FHIR Address element captured for a patient, supporting multiple addresses by use/type and validity period';
COMMENT ON COLUMN insaights_patient_address.address_id IS 'Unique identifier for the address record (FHIR Address.id when persisted)';
COMMENT ON COLUMN insaights_patient_address.use IS 'Purpose of the address (FHIR Address.use), such as home, work, temporary, old, or billing';
COMMENT ON COLUMN insaights_patient_address.city IS 'City, town, or locality (FHIR Address.city)';
COMMENT ON COLUMN insaights_patient_address.state IS 'State, province, or region (FHIR Address.state)';
COMMENT ON COLUMN insaights_patient_address.postal_code IS 'Postal or ZIP code (FHIR Address.postalCode)';
COMMENT ON COLUMN insaights_patient_address.country IS 'Country name (FHIR Address.country)';

----------------------------
-- 10. insaights_communication
----------------------------
...
ALTER TABLE insaights_communication RENAME TO insaights_patient_communication;

COMMENT ON TABLE insaights_communication IS 'FHIR Patient.communication element storing patient language preferences and communication requirements';
COMMENT ON COLUMN insaights_communication.communication_id IS 'Unique identifier for the patient communication preference record';
COMMENT ON COLUMN insaights_communication.language_code IS 'Language code representing the patient’s spoken or preferred language (FHIR Patient.communication.language)';
COMMENT ON COLUMN insaights_communication.preferred IS 'Indicates whether this language is marked as the patient’s preferred language';

-------------------------------------
-- 11. insaights_patient_contact
-------------------------------------
CREATE TABLE insaights_patient_contact (
    contact_id UUID PRIMARY KEY,
    patient_id UUID NOT NULL REFERENCES insaights_patient(patient_id),
    name_prefix TEXT, first_name TEXT, middle_name TEXT, last_name TEXT, name_suffix TEXT, name_text TEXT,
    address_use TEXT, address_type TEXT, address_text TEXT,
    address_line1 TEXT, address_line2 TEXT, address_line3 TEXT, address_line4 TEXT,
    address_city TEXT, address_district TEXT, address_state TEXT, address_postal_code TEXT, address_country TEXT,
    address_period_start TIMESTAMPTZ, address_period_end TIMESTAMPTZ,
    gender TEXT,
    organization_id UUID REFERENCES insaights_organisation(org_id),
    period_start TIMESTAMPTZ,
    period_end TIMESTAMPTZ
...
COMMENT ON TABLE insaights_patient_contact IS 'FHIR Patient.contact element representing one or more contact persons for a patient, such as next-of-kin or emergency contacts';
COMMENT ON COLUMN insaights_patient_contact.contact_id IS 'Unique identifier for the patient contact record';
COMMENT ON COLUMN insaights_patient_contact.patient_id IS 'Reference to the patient to whom this contact is associated';
COMMENT ON COLUMN insaights_patient_contact.organization_id IS 'Reference to an organisation associated with the contact person';

-------------------------------------
-- 12. insaights_patient_contact_relationship
-------------------------------------
CREATE TABLE insaights_patient_contact_relationship (
    contact_relationship_id UUID PRIMARY KEY,
    relationship_code TEXT,
    contact_id UUID NOT NULL REFERENCES insaights_patient_contact(contact_id)
);

COMMENT ON TABLE insaights_patient_contact_relationship IS 'FHIR Patient.contact.relationship element allowing a single contact to carry multiple relationship or role codes with respect to the patient';
COMMENT ON COLUMN insaights_patient_contact_relationship.relationship_code IS 'Relationship or role code describing the contact’s association to the patient (e.g., emergency contact, next-of-kin, billing)';

-------------------------------------
-- 13. insaights_patient_contact_telecom
-------------------------------------
CREATE TABLE insaights_patient_contact_telecom (
    contact_telecom_id UUID PRIMARY KEY,
    system TEXT, value TEXT, use TEXT, rank INTEGER,
    period_start TIMESTAMPTZ, period_end TIMESTAMPTZ,
    contact_id UUID NOT NULL REFERENCES insaights_patient_contact(contact_id),
    CONSTRAINT chk_contact_telecom_rank_non_negative CHECK (rank IS NULL OR rank >= 0)
);

COMMENT ON TABLE insaights_patient_contact_telecom IS 'FHIR Patient.contact.telecom element storing multiple communication points for a contact person';
COMMENT ON COLUMN insaights_patient_contact_telecom.system IS 'Telecom system type (FHIR ContactPoint.system), such as phone, fax, email, pager, URL, SMS, or other';

----------------------------
-- 14. insaights_related_person
----------------------------
...
COMMENT ON TABLE insaights_related_person IS 'FHIR RelatedPerson resource representing individuals involved in a patient’s care, such as family members or caregivers';
COMMENT ON COLUMN insaights_related_person.related_person_id IS 'Unique identifier for the related person resource (FHIR RelatedPerson.id)';

----------------------------
-- 15. insaights_patient_link
----------------------------
CREATE TABLE insaights_patient_link (
    patient_link_id UUID PRIMARY KEY,
    other_patient_id UUID REFERENCES insaights_patient(patient_id),
    related_person_id UUID REFERENCES insaights_related_person(related_person_id),
    type TEXT NOT NULL,
    patient_id UUID NOT NULL REFERENCES insaights_patient(patient_id),
    CONSTRAINT chk_patient_link_one_target CHECK (
        (other_patient_id IS NOT NULL AND related_person_id IS NULL) OR
        (other_patient_id IS NULL AND related_person_id IS NOT NULL)
    )
);

COMMENT ON TABLE insaights_patient_link IS 'FHIR Patient.link element used to connect a patient to another patient record or to a related person for deduplication, merging, and cross-referencing';

---------------------------------
-- 16. insaights_general_practitioner
---------------------------------
CREATE TABLE insaights_general_practitioner (
    general_practitioner_id UUID PRIMARY KEY,
    organisation_id UUID REFERENCES insaights_organisation(org_id),
    practitioner_id UUID REFERENCES insaights_practitioner(practitioner_id),
    practitioner_role_id UUID REFERENCES insaights_practitioner_role(practitioner_role_id),
    patient_id UUID NOT NULL REFERENCES insaights_patient(patient_id),
...
COMMENT ON TABLE insaights_general_practitioner IS 'Association table linking patients to their general practitioner reference, which may be a practitioner, organisation, or practitioner role';
COMMENT ON COLUMN insaights_general_practitioner.patient_id IS 'Reference to the patient to whom the general practitioner association applies';
COMMENT ON CONSTRAINT chk_gp_only_one_reference ON insaights_general_practitioner IS 'Ensures exactly one general practitioner reference is provided: practitioner, organisation, or practitioner role';
...
-------------------------------------
-- 42. insaights_encounter_reason_value
-------------------------------------
...
-------------------------------------
-- 43. insaights_encounter_diagnosis
-------------------------------------
...
-------------------------------------
-- 44. insaights_encounter_diagnosis_condition
-------------------------------------
...
-------------------------------------
-- 45. insaights_encounter_diagnosis_use
-------------------------------------
...
-------------------------------------
-- 46. insaights_encounter_admission
-------------------------------------
...
-------------------------------------
-- 47. insaights_encounter_location
-------------------------------------
CREATE TABLE insaights_encounter_location (
    ...
    CONSTRAINT fk_encounter_location_location
        FOREIGN KEY (location_id) REFERENCES insaights_location(id),
...
-------------------------------------
-- 48. insaights_encounter_police_report
-------------------------------------
